package com.cpigeon.app.utils.video;
...
import com.cpigeon.app.utils.video.camera.SensorControler;
import com.cpigeon.app.utils.video.widget.CameraView;
import com.cpigeon.app.utils.video.widget.CircularProgressView;
import com.cpigeon.app.utils.video.widget.FocusImageView;
...
//                        Bitmap viewWatermarkBitmap1 = BitmapUtils.getViewBitmap(ac_time);//控件转化成有水印的bipmap
                            Bitmap viewWatermarkBitmap1 = BitmapUtil.convertViewToBitmap(watermarkLlz);//控件转化成有水印的bipmap
...
//                }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                float rawY = sRawY * ScreenTool.getScreenWidth(this) / ScreenTool.getScreenHeight(this);
                float temp = sRawX;
                float rawX = rawY;
                rawY = (ScreenTool.getScreenWidth(this) - temp) * ScreenTool.getScreenHeight(this) / ScreenTool.getScreenWidth(this);
    ...
    public void onFocus() {
        ...
        Point point = new Point(ScreenTool.getScreenWidth(this) / 2, ScreenTool.getScreenWidth(this) / 2);
        mCameraView.onFocus(point, callback);
    }
...
//                                Bitmap bitmap1 = BitmapUtils.createBitmapCenter(BitmapUtils.createBitmapLowerLeft(BitmapUtils.rotaingImageView(90, BitmapFactory.decodeByteArray(data, 0, data.length)), BitmapUtils.convertViewToBitmap(watermarkLlz)), BitmapUtils.convertViewToBitmap(waterCenImg));
                                ...
                                //图片保存
                                BitmapUtil.saveJPGE_After(RecordedActivity.this, BitmapUtil.createBitmapCenter(BitmapUtil.createBitmapLowerLeft(BitmapUtil.rotaingImageView(90, BitmapFactory.decodeByteArray(data, 0, data.length)), BitmapUtil.convertViewToBitmap(watermarkLlz)), BitmapUtil.convertViewToBitmap(waterCenImg)), img_path, 100);
                                Intent intent1 = null;
                                if (type.equals("sgt")) {
                                  /*  intent1 = new Intent(RecordedActivity.this, SGTImgUploadActivity.class);
                                    intent1.putExtra("SGTHomeListEntity", getIntent().getSerializableExtra("SGTHomeListEntity"));
                                    intent1.putExtra("DataBean", getIntent().getSerializableExtra("DataBean"));
                                    intent1.putExtra("code", 2);//1：图片选择  2：拍摄图片
                                    intent1.putExtra("uplodType", getIntent().getIntExtra("uplodType", -1));*///1：单个足环  2：鸽主

                                } else if (type.equals("photo")) {
                                    //intent1 = new Intent(RecordedActivity.this, PhotoEditActivity.class);
                                }
